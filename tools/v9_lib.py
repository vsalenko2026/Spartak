...
def период(с_года, по_год=None):
    """Формула «09.2026 – 08.2027»: какой кусок календаря закрывает год.

    «Год 1» сам по себе не говорит ничего, а строка «Месяц» показывает в
    колонке года последний месяц — и читается как будто год и есть август
    2027. Период в шапке снимает вопрос.

    Месяцы собираются из чисел, а не через TEXT(дата;"ММ.ГГГГ"): код
    формата внутри TEXT разбирается языком самого Excel, и русский шаблон
    в английской раскладке печатается как есть."""
    по_год = по_год or с_года
    старт = f"DATE(YEAR(СтартМодели),MONTH(СтартМодели)+{12 * (с_года - 1)},1)"
    конец = f"DATE(YEAR(СтартМодели),MONTH(СтартМодели)+{12 * по_год - 1},1)"
    return (f'=TEXT(MONTH({старт}),"00")&"."&YEAR({старт})&" – "'
            f'&TEXT(MONTH({конец}),"00")&"."&YEAR({конец})')
...
class Лист:
    ...
    def шапка(self, дата_строка=None):
        ...
        for y in range(1, ЛЕТ + 1):
            ...
            ws[f"{год(y)}{rд}"] = период(y)
        ws[f"{L(КОЛ_ИТОГО)}{r}"] = f"Итого {ЛЕТ} лет"
        ws[f"{L(КОЛ_ИТОГО)}{rд}"] = период(1, ЛЕТ)
        for i in range(1, ВСЕГО_МЕСЯЦЕВ + 1):
            c = мес(i)
        ...
        # подписи слева занимают обе строки шапки; у колонок лет вторая
        # строка занята периодом
        for col in range(2, КОЛ_ГОД1):
    ...
    def ряд(self, название, формула, ед="₽", как="", формат=ДЕНЬГИ,
        ...
        `итог="доля"` вместе с `делимое`/`делитель` считает годовую колонку
        как отношение годовых сумм: долю усреднять по месяцам нельзя."""
        ws, r = self.ws, self.строка
        ...
        for y in range(1, ЛЕТ + 1):
            ...
            ф = (f"=SUM({a}{r}:{b}{r})" if итог == "сумма"
                 ...
                 else (f"=IF(SUM({a}{делитель}:{b}{делитель})=0,0,"
                       f"SUM({a}{делимое}:{b}{делимое})"
                       f"/SUM({a}{делитель}:{b}{делитель}))")
                 if итог == "доля"
                 else f"=AVERAGE({a}{r}:{b}{r})")
            ws[f"{год(y)}{r}"] = ф
            ws[f"{год(y)}{r}"].number_format = формат
        м1, мN = мес(1), мес(МЕСЯЦЕВ)
        итого = (f"=SUM({м1}{r}:{мN}{r})" if итог == "сумма"
                 else f"={мN}{r}" if итог == "конец"
                 else f"=MIN({м1}{r}:{мN}{r})" if итог == "минимум"
                 else (f"=IF(SUM({м1}{делитель}:{мN}{делитель})=0,0,"
                       f"SUM({м1}{делимое}:{мN}{делимое})"
                       f"/SUM({м1}{делитель}:{мN}{делитель}))")
                 if итог == "доля"
                 else f"=AVERAGE({м1}{r}:{мN}{r})")
