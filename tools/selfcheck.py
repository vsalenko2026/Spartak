...
DEFECTS = {
    ...
    # Одного нуля в пороге для минуса на счёте уже мало: резерв на
    # ближайшую просадку (11.09.2026) держит счёт и без порога. Поэтому
    # дефект ловит теперь сама проверка порога и эталон, а не минус в
    # кассе — минус ловится дефектом «просадка» ниже.
    "пустой счёт": (
    ...
    "просадка": (
        "резерв на ближайшую просадку обнулён — порог остатка снова "
        "проверяется только в месяц платежа клубу",
        {f"02_Модель!{c}94": 0 for c in СТОЛБЦЫ_МЕСЯЦЕВ},
        ["test_порог_держится_после_первого_платежа_клубу"],
    ),
    "порог займа": (
        "порог возврата займа поднят выше целевого запаса",
        {"01_Вводные!B310": 4},
        ["test_порог_возврата_займа_не_выше_целевого_запаса"],
    ),
    "разом": (
